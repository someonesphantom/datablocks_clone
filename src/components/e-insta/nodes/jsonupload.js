...
const Jsoninputnode = ({ data }) => {
    ...
    const HandleChange = (event) => {
        ...
        console.log("the event is", event.target.files[0].name)
        ...
        axios.post(apiMapping.userData.uploadjson, formData, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        }).then(response => {
            // console.log("response for json", response.data);
            // let recievedpayload = JSON.parse(response.data[0]);
            // console.log("recievedpyload",[recievedpayload])
